...
TWITTER_SCREEN_NAME = "greenestcity"
...
num_mentioned_by_greenestcity_that_are_following_greenestcity = 0
usersColl = db.collection("users")
tweetsColl = db.collection("tweets")

# 15339443 is the user id for greenestcity
tweetsColl.find({"user.id" => 15339443}, :fields => ["id", "entities"]).each do |t|
  t["entities"]["user_mentions"].each do |u|
    ...
    if user["partial_following_screen_names"].include?("greenestcity")
      $stderr.printf("user id:%d is following greenestcity and was mentioned by greenestcity\n", id_of_user_mentioned)
      num_mentioned_by_greenestcity_that_are_following_greenestcity += 1
    end
  end
end

printf("num_mentioned_by_greenestcity_that_are_following_greenestcity:%d\n", num_mentioned_by_greenestcity_that_are_following_greenestcity)
